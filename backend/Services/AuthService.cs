...
public class AuthService : IAuthService
{
    ...
    /// <summary>
    /// Register new user — SCRUM-17: AC-B1 through AC-B9
    /// Returns: (Success, Response, FieldLevelErrors)
    /// FieldLevelErrors: Dictionary of field names to error messages (AC-B2, AC-B9)
    /// </summary>
    public async Task<(bool Success, RegisterResponse Response, Dictionary<string, string> Errors)> RegisterUserAsync(RegisterRequest request)
    {
        var errors = new Dictionary<string, string>();

        // Step 1: Validate required fields exist — AC-B9
        ...
        // If required fields missing, return immediately
        if (errors.Any())
        {
            return (false, new RegisterResponse(), errors);
        }

        // Step 2: Validate email format — AC-B2
        if (!System.Text.RegularExpressions.Regex.IsMatch(request.Email.Trim(), @"^[^\s@]+@[^\s@]+\.[^\s@]+$"))
        {
            errors["email"] = "Invalid email format";
            return (false, new RegisterResponse(), errors);
        }

        // Step 3: Check email uniqueness — AC-B2
        var existingUser = await _db.Users.FirstOrDefaultAsync(u => u.Email == request.Email.Trim().ToLower());
        ...
        // Step 4: Validate password/confirmPassword match — AC-B3
        ...
        // Step 5: Validate password minimum length — AC-B4
        ...
        try
        {
            // Step 6: Hash password using BCrypt — AC-B5
            var passwordHash = BCrypt.Net.BCrypt.HashPassword(request.Password, 12);

            // Step 7: Create User entity — AC-B6
            var user = new User
            {
                Id = Guid.NewGuid(),
                Name = request.Name.Trim(),
                Username = request.Email.Trim().ToLower(), // Use email as username
            ...
            // Step 8: Save to database — AC-B6
            _db.Users.Add(user);
            await _db.SaveChangesAsync();

            // Step 9: Log registration in audit log
            ...
            // Step 10: Send verification email — AC-B8
            _ = Task.Run(async () =>
            {
                try
                {
            ...
            // Step 11: Return 201 Created response with message (no passwordHash) — AC-B7
            ...
            return (true, response, new Dictionary<string, string>());
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Registration error");
            var errorDict = new Dictionary<string, string> { { "general", "An internal error occurred during registration." } };
            return (false, new RegisterResponse { Success = false }, errorDict);
        }
    }
