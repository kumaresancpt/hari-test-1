...
public class AuthController : ControllerBase
{
    ...
    /// <summary>Register — SCRUM-17: AC-B1 through AC-B9: User registration endpoint</summary>
    [HttpPost("register")]
    ...
    public async Task<IActionResult> Register([FromBody] RegisterRequest request)
    {
        try
        {
            // AC-B9: Check for required fields via ModelState
            if (!ModelState.IsValid)
            {
                ...
                foreach (var kvp in ModelState)
                {
                    ...
                    {
                        validationErrors[kvp.Key] = error.ErrorMessage;
                    }
                }
            ...
            var (success, response, fieldErrors) = await _auth.RegisterUserAsync(request);

            if (success)
            {
                // AC-B7: Return 201 Created with success message (no passwordHash)
                return CreatedAtAction(nameof(Register), new { userId = response.UserId }, response);
            }
            else
            {
                // AC-B2, AC-B3, AC-B4, AC-B9: Return 400 with field-level error details
                return BadRequest(new ErrorResponse 
                { 
